// Fondo animado del hero del home — puerto sin React del componente
// "Mesh drift" (21st.dev Shader Builder). Un solo canvas WebGL, sin
// dependencias, con la paleta de marca de JEM Store.
//
// Si WebGL no está disponible o el usuario prefiere menos movimiento
// (prefers-reduced-motion), no se activa y queda la imagen estática
// de respaldo que ya trae el markup.
...
// Paleta de marca JEM (ver resources/sass/_variables.scss), normalizada a 0..1.
const JEM_COLORS = [
  [0.0863, 0.0863, 0.0863], // $jem-obsidian #161616
  [0.1255, 0.1686, 0.2745], // $jem-ink       #202b46
  [0.4667, 0.2196, 0.2745], // $jem-merlot    #773846
  [0.7686, 0.8471, 0.9412], // $jem-sky       #c4d8f0
  [0.7686, 0.8471, 0.9412],
  [0.7686, 0.8471, 0.9412],
  [0.7686, 0.8471, 0.9412],
  [0.7686, 0.8471, 0.9412],
]
...
export function initHeroShader() {
  const visual = document.querySelector('[data-shader-hero]')
  const canvas = visual?.querySelector('.jem-hero-shader')
  if (!visual || !canvas) return

  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
  if (prefersReducedMotion) return

  // El canvas se muestra ANTES de inicializar WebGL: mountShaderBackground()
  // mide bounds.width/height del canvas para dimensionarlo, y con
  // display:none esas medidas dan 0x0 (canvas 1x1, en la práctica invisible).
  // Si falla, se revierte a la imagen de respaldo.
  visual.classList.add('jem-shader-active')

  const dispose = mountShaderBackground(canvas)
  if (!dispose) {
    visual.classList.remove('jem-shader-active')
  }
}
